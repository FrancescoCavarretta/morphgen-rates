...
def plot_two_curves_with_errorbars(x1, y1, err1, x2, y2, err2, 
                                    ...
                                    significant_bins1=[], significant_bins2=[], marker_height1=0.1, marker_height2=10, xlim=[-5, 725], ylim=[0, 25]):
    
    return plot_three_curves_with_errorbars(x1, y1, err1, x2, y2, err2, y3=None, err3=None,
                                    label1=label1, label2=label2, 
                                    color1=color1, color2=color2, 
                                    xlabel=xlabel, ylabel=ylabel, title=title,
                                    significant_bins1=significant_bins1, significant_bins2=significant_bins2, marker_height1=marker_height1, marker_height2=marker_height2, xlim=xlim, ylim=ylim)
...
def plot_three_curves_with_errorbars(x1, y1, err1, x2, y2, err2, x3=None, y3=None, err3=None,
    ...
    plt.gca().spines['top'].set_visible(False)
    plt.gca().spines['right'].set_visible(False)
    plt.xlim([xlim[0]-25,xlim[1]+25])
    plt.xticks(np.arange(xlim[0], xlim[1]+50, 50))
    plt.ylim(ylim)
...
if __name__ == "__main__":
    ...
    data_bif = {
        'PYR':{'Mean':18.8, 'SD':4.3, 'Samples':[23, 30, 18, 21, 19, 18, 17, 15, 14, 15, 15, 21]},
        'SL':{'Mean':18.0, 'SD':9.9, 'Samples':[24, 26, 4]},
        'hippocampus':{'Mean':55.8, 'SD':11.6, 'Samples': [61, 49, 47, 59, 43, 45, 62, 65, 66, 48, 79, 68, 65, 62, 63, 61, 56, 40, 51, 70, 46, 64, 60, 69, 35, 59, 54, 55, 66, 45, 63, 78, 33, 57, 61, 41, 39, 35, 53, 47, 52, 61, 54, 70, 46, 60, 65, 73, 42, 41, 53, 79, 50, 64, 51, 50, 70, 71, 52, 56, 65, 53, 61, 52, 46, 41, 59, 53, 49, 64, 54, 61, 69, 56, 46, 63, 43, 59, 59, 59, 52, 33, 59, 63, 54, 48, 52, 51, 38, 71, 55, 67, 51, 37, 49, 36, 58, 63, 50, 53, 62, 75, 59, 44, 71, 48, 53, 110, 48, 53, 51, 39, 50, 41, 56, 83, 67, 54, 61, 54, 61, 59, 49, 54, 58, 46, 41, 52, 81, 46, 71, 64, 70, 61, 40, 57, 36, 59, 59, 42]},
        'neocortex':{'Mean':60.8, 'SD':28.5, 'Samples':[75, 44, 86, 93, 182, 70, 33, 30, 59, 93, 77, 57, 64, 56, 44, 29, 74, 71, 50, 84, 36, 44, 44, 71, 49, 48, 38, 37, 58, 38, 38, 73]}
        }[cell_type]
    ...
    hist_xlim, hist_ylim, sholl_xlim, sholl_ylim = {
        'SL':([0, 80], [0, 100], [0, 700], [0, 20]),
        'PYR':([0, 80], [0, 100], [0, 700], [0, 20]),
        'hippocampus':([0, 160], [0, 100], [0, 600], [0, 40]),
        'neocortex':([0, 160], [0, 100], [0, 1200], [0, 40])
         }[cell_type]
    ...
    filepath = {'PYR':'pyr_apical_sholl_plot.txt', 'SL':'sl_apical_sholl_plot.txt', 'hippocampus':'hippocampus_apical_sholl_plot.txt', 'neocortex':'neocortex_apical_sholl_plot.txt'}[cell_type] # file containing sholl plots
    ...
    print('\nsim vs exp')
    print(r, '\n\n')
    significant_bins1 = r[r.SD < alpha / r.size].index
...
##    r = two_samples_tests(bif_mean_sim, bif_std_sim ** 2, n_trials, bif_mean_exp, bif_std_exp ** 2, n_exp_neuron)
    ...
    plot_two_curves_with_errorbars(sim_data.index, sim_data.Mean, sim_data.SD, exp_data.index, exp_data.Mean, exp_data.SD, significant_bins1=significant_bins1, significant_bins2=significant_bins2, xlim=sholl_xlim, ylim=sholl_ylim)
